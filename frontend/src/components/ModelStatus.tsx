  ...
  return (
    <section className="surface grid gap-3 p-4">
      <div className="flex items-center gap-2 text-sm font-medium text-deep-950 dark:text-white">
        <Cpu className="h-4 w-4 text-teal-500" aria-hidden="true" />
        Translation Engine
      </div>
      <div className="grid grid-cols-1 gap-3 text-sm sm:grid-cols-3">
        <div className="stat-card">
          <div className="flex items-center gap-2 text-ink-500 dark:text-blue-50/60">
      ...
      <p className="text-sm text-ink-600 dark:text-blue-50/70">{state.message}</p>
      {warnings.length > 0 ? (
        <ul className="space-y-1 text-sm text-ink-600 dark:text-blue-50/70">
