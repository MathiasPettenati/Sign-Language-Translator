...
export function HistoryPanel({ items }: HistoryPanelProps) {
  return (
    <section className="surface p-4">
      ...
      {items.length === 0 ? (
        <p className="surface-muted mt-4 p-3 text-sm text-ink-600 dark:text-blue-50/70">
          ...
          {items.map((item) => (
            ...
            >
              <div>
                ...
                <p className="text-xs text-ink-500 dark:text-blue-50/60">{item.source}</p>
              </div>
              <div className="text-right text-xs text-ink-500 dark:text-blue-50/60">
